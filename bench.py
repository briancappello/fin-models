...
if __name__ == "__main__":
    ...
    """
    to_parquet = timeit(lambda: amd.to_parquet("/tmp/amd.parquet", engine='pyarrow'), number=LOOPS)
    print(f'pyarrow {to_parquet/LOOPS=}s')

    from_parquet = timeit(lambda: pd.read_parquet("/tmp/amd.parquet", engine='pyarrow'), number=LOOPS)
    print(f'pyarrow {from_parquet/LOOPS=}s')

    to_parquet = timeit(lambda: amd.to_parquet("/tmp/amd.parquet", engine='fastparquet'), number=LOOPS)
    print(f'fast {to_parquet/LOOPS=}s')

    from_parquet = timeit(lambda: pd.read_parquet("/tmp/amd.parquet", engine='fastparquet'), number=LOOPS)
